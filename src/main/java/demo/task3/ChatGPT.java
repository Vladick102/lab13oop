...
public class ChatGPT {
  ...
  public Company extractCompanyDetails(String scrapedData) {
    try {
      ...
      // Create the prompt
      String prompt =
          "Extract the company name, description, and logo URL from the following data:\n"
              + scrapedData
      ...
      try (Scanner scanner = new Scanner(connection.getInputStream(), StandardCharsets.UTF_8)) {
        ...
        String responseText =
            jsonResponse.getJSONArray("choices").getJSONObject(0).getString("text").trim();
        JSONObject companyDetails = new JSONObject(responseText);

        // Create and return the Company object
        return new Company(companyDetails.optString("name", "N/A"),
